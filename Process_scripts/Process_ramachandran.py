...
for molec in molecList : 
    ...
    try : 
        datafile = glob.glob('B_State/%s/chi/ramaPhiPsiCNF*.xvg'%(molec))[0]
    except IndexError : 
        print "ERROR: Failed to find file 'B_State/%s/chi/ramaPhiPsiCNF*.xvg"%(molec)
        continue 
    resIndex = os.path.splitext(basename(datafile))[0]
    resIndex = resIndex.split("CNF")[1]

    datafile2 = glob.glob("B_State/GFP_WT/chi/ramaPhiPsi???%s.xvg"%(resIndex))[0]
...
for molec in molecList : 
    ...
    try : 
        datafile = glob.glob('B_State/%s/chi/ramaX1X2CNF*.xvg'%(molec)) 
    except IndexError : 
        print "ERROR: Failed to find file 'B_State/%s/chi/ramaPhiPsiCNF*.xvg"%(molec)
        continue 
    ...
    try : 
        print datafile
        data = np.genfromtxt(datafile[0],skip_header=41) 
    except IOError : 
        print "%s not found!"%datafile

    try : 
        datafile2 = glob.glob("B_State/GFP_WT/chi/ramaX1X2???%s.xvg"%(resIndex))[0]
        data2 = np.genfromtxt(datafile2,skip_header=41) 
