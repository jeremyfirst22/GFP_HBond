...
for molec in molecList : 
    ...
    try : 
        datafile = glob.glob('B_State/%s/chi_290/ramaPhiPsiCNF*.xvg'%(molec))[0]
    except IndexError : 
        print "ERROR: Failed to find file 'B_State/%s/chi_290/ramaPhiPsiCNF*.xvg"%(molec)
        continue 
    resIndex = os.path.splitext(basename(datafile))[0]
    resIndex = resIndex.split("CNF")[1]

    datafile2 = glob.glob("B_State/GFP_WT/chi_290/ramaPhiPsi???%s.xvg"%(resIndex))[0]
    ...
    index +=1
fig.savefig('figures/ramaPhiPsi_290.png',format='png') 
...
for molec in molecList : 
    ...
    try : 
        datafile = glob.glob('B_State/%s/chi_290/ramaX1X2CNF*.xvg'%(molec)) 
    except IndexError : 
        print "ERROR: Failed to find file 'B_State/%s/chi_290/ramaPhiPsiCNF*.xvg"%(molec)
        continue 
    ...
    try : 
        datafile2 = glob.glob("B_State/GFP_WT/chi_290/ramaX1X2???%s.xvg"%(resIndex))[0]
        data2 = np.genfromtxt(datafile2,skip_header=43) 
    ...
    index +=1
fig.savefig('figures/ramaX1X2_290.png',format='png') 
