...
for index,molec in enumerate(molecList) : 
    file="B_State/GFP_%s/hbond_with_ca_310/geometry.xvg"%molec
...
fig.savefig('figures/Geometries_heat_310.png',format='png') 
plt.close() 
...
for index,molec in enumerate(molecList) : 
    file="B_State/GFP_%s/hbond_with_ca_310/nw_geometry.xvg"%molec
    data = np.genfromtxt(file,skip_header=23) 
    try :
        data[:,0] = data[:,0] / 1000 * 4
    except IndexError : 
...
fig.savefig('figures/Geometries_protein_heat_310.png',format='png') 
plt.close() 
...
absMax = {} 
with open('Exp_data/sasa_abs_data_310.dat') as f : 
    for line in f : 
...
accumAbsMax = [] 
for index, molec in enumerate(molecList) : 
    fileWhis = 'B_State/GFP_%s/fit_hbond_with_ca_310/theta1.his'%(molec) 
    fileWpoly = 'B_State/GFP_%s/fit_hbond_with_ca_310/theta1.poly'%(molec) 
    fileWgeom = 'B_State/GFP_%s/fit_hbond_with_ca_310/geometry.xvg'%(molec) 
    filePhis = 'B_State/GFP_%s/fit_hbond_with_ca_310/nw_theta1.his'%(molec) 
    filePpoly = 'B_State/GFP_%s/fit_hbond_with_ca_310/nw_theta1.poly'%(molec) 
    filePgeom = 'B_State/GFP_%s/fit_hbond_with_ca_310/nw_geometry.xvg'%(molec) 
    ...
    if not molec == "N212X" : 
        ...
        avgAngleTot = avgAngleW / sum(probsW) 
...
ax2.legend(loc=4) 
fig.savefig('figures/Geometries_angles_310.png',format='png') 
figDens.savefig('figures/Geometries_angles_density_310.png',format='png') 
f2.savefig('figures/abs_max_vs_max_theta_310.pdf',format='pdf') 
plt.close() 
